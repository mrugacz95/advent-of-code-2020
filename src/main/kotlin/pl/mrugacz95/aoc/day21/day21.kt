...
import pl.mrugacz95.aoc.day16.HopcroftKarpSolver
...
fun solve(foodList: List<Food>): Map<String, String> {
    ...
    return HopcroftKarpSolver<String, String>().solve(mayBeIn.toMap())
}
...
fun main() {
    ...
    println(
        "Answer part 1: ${
            food.map { f -> f.ingredients.filter { ingredient -> ingredient !in matching.values }.count() }.sum()
        }"
    )
